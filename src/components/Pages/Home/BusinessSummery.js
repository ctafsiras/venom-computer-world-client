import { CurrencyDollarIcon, ShoppingCartIcon } from '@heroicons/react/outline';
import React from 'react';

const BusinessSummery = () => {
    return (
        <div>

            <div>
                <h2 className='text-primary text-3xl font-bold text-center mt-6'>Our Success Summery</h2>
                <h2 className='text-secondary text-xl font-bold text-center mt-2'>V.C.W. Growth State</h2>
                <div className="divider"></div>
            </div>

            <div className="stats shadow mx-auto w-full">


                <div className="stat place-items-center">
                    <div className="stat-title">Sales</div>
                    <ShoppingCartIcon  className='w-16'></ShoppingCartIcon>
                    
                    <div className="stat-value">21K</div>
                    <div className="stat-desc">Since 2020</div>
                </div>

                <div className="stat place-items-center">
                    <div className="stat-title">Registered Users</div>
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-16 w-16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                    </svg>
                    <div className="stat-value text-secondary">780</div>
                    <div className="stat-desc text-secondary">Increases 5% Per Year</div>
                </div>

                <div className="stat place-items-center">
                    <div className="stat-title">Revenue</div>
                    <CurrencyDollarIcon className='w-16' />
                    <div className="stat-value">34K</div>
                    <div className="stat-desc">Increases 15% Per Year</div>
                </div>

            </div>
        </div>
    );
};

export default BusinessSummery;